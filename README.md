...
(Suggested: Screenshot of your hand-drawn or digital diagram showing Source → Kafka → Spark → Storage → Airflow)
...
Watermarking: A 10-minute watermark is implemented to handle out-of-order data.

Logic: Spark holds the state of a user's location for 10 minutes. If a transaction is delayed due to poor connectivity, it is still accurately processed against the "Impossible Travel" rule.

Mathematical Threshold: Threshold = max(eventTime) - 10 minutes
...
Key Insight: The highest fraud frequency occurred in Travel. This aligns with our detection algorithm, as fraudsters often target travel bookings for high-value liquidations that are difficult to reverse.
...
Anonymization: User IDs should be hashed (e.g., salted SHA-256) so the engine detects patterns without knowing real-world identities.

Data Minimization: Only city-level location data is stored, rather than precise GPS coordinates.

Retention Policy: In compliance with GDPR, transaction logs are moved to a "Warehouse" with a 7-year purge policy for financial auditing compliance.
...
Validation Formula: Total Ingress == Validated Amount + Fraud Amount

Current Run Status: Successfully Balanced.


🚀 How to Run
Initialize Stack: docker-compose up -d

Start Stream: spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.x fraud_detection.py
